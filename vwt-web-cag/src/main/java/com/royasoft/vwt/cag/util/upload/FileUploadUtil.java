...
public class FileUploadUtil {
     ...
     * @Description:
     */
    public static JSONObject wirteFileToLocal( byte[] fileBytes,String fileName) throws Exception {
        logger.debug(" 写byte数组zip到 本地文件zip fileBytes  :{},fileName:{} ,",fileBytes == null ? 0 : fileBytes.length, fileName);
    ...
    public static String uploadFile(Object msg) throws Exception {
        ...
        if ("jsp".equalsIgnoreCase(extName) || "asp".equalsIgnoreCase(extName) || "js".equalsIgnoreCase(extName)) {
            //因为系统漏洞扫描时，发现可以上传xxx.jsp这个的脚本文件，所以此处加判断处理
            return "";
        } else {
