...
@Scope("prototype")
@Service
public class VersionUpdateVGPService implements Runnable {
    ...
    public void run() {
        while (true) {
            try {
                queue_packet = ServicesQueue.versionVGP_queue.take();// 获取队列处理数据
                msg = queue_packet.getMsg();// 获取请求信息
                channel = queue_packet.getChannel();// 获取连接
                logger.debug("version_queue:{}", ServicesQueue.versionVGP_queue.size());
                if (msg instanceof HttpRequest) {
                    ...
                    logger.debug("VGP版本更新处理类(入口),function_id:{},tel_number:{},request_body:{}", function_id, tel_number, request_body);
                    /***************************** 业务逻辑处理 *********************************************/
                    ...
                    logger.debug("VGP版本更新处理类(响应),function_id:{},user_id:{},request_body:{},resInfo:{}", function_id, tel_number, request_body, resInfo);
                    ...
                    if (null != responseStatus && !"".equals(responseStatus))
                        operationLogService.saveOperationLogNew(channel, request, "", tel_number, function_id, request_body, "", responseStatus);
                    continue;
                }
                ResponsePackUtil.cagHttpResponse(channel, ResponsePackUtil.buildPack(ResponseInfoConstant.FAIL1004, ""));
            } catch (Exception e) {
                logger.error("VGP版本更新处理类异常", e);
                // 响应客户端异常
                ResponsePackUtil.cagHttpResponse(channel, ResponsePackUtil.buildPack(ResponseInfoConstant.FAIL1004, ""));
            } finally {
